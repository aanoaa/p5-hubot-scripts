...
sub load {
    ...
    $robot->respond(
        ...
        sub {
            ...
            if (my ($user) = grep /^$recipient$/i, keys %{$robot->brain->{data}{users}}) {
                ...
                $robot->brain->{data}{tell}{$recipient}{+time} = [ $sender, $recipient, $post, $dt->ymd." ".$dt->hms ];
    ...
    $robot->enter(
        sub {
            ...
            for my $recipient (keys %{$robot->brain->{data}{tell}}) {
                if ($user =~ /^$recipient$/i) {
                    for my $post (values %{$robot->brain->{data}{tell}{$recipient}}) {
                        $msg->send( "$user: $post->[3]: <$post->[0]> $post->[2]" );
                    }
